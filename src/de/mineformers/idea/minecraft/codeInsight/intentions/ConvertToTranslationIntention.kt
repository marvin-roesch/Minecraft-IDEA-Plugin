package de.mineformers.idea.minecraft.codeInsight.intentions
    ...
    override fun invoke(project: Project, editor: Editor, element: PsiElement) {
        if (element.parent is PsiLiteral) {
            ...
            val result = Messages.showInputDialogWithCheckBox("Enter translation key:",
                                                              ...
                                                              "Replace literal with call to StatCollector",
                                                              true,
                                                              true,
            ...
            if (result.getFirst() != null) {
                I18nElementFactory.addTranslation(ProjectRootManager.getInstance(project).fileIndex.getModuleForFile(
                    FileDocumentManager.getInstance().getFile(editor.document)!!), result.getFirst(), value)
                if (result.getSecond())
                    object : WriteCommandAction.Simple<Unit>(project,
                                                             PsiDocumentManager.getInstance(project).getPsiFile(editor.document)) {
                        @Throws(Throwable::class)
                        override fun run() {
                            val expression = JavaPsiFacade.getElementFactory(project).createExpressionFromText("net.minecraft.util.StatCollector.translateToLocal(\"" + result.getFirst() + "\")",
                                                                                                               element.context)
